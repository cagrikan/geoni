...
import task_protection as worker   # ayni testler, boto3'suz modul
...
def _izle(monkeypatch):
    ...
    monkeypatch.setattr(worker, "set_protection", sahte)
    monkeypatch.setattr(worker, "_count", 0)
    return cagrilar
...
def test_paralel_iki_is_arada_korumayi_dusurmez(monkeypatch):
    ...
    async def akis():
        await worker.acquire()   # is A
        await worker.acquire()   # is B
        await worker.release()   # A bitti — koruma KALMALI
        ara = list(c)
        await worker.release()   # B bitti — simdi kalkar
        return ara
...
def test_sayac_negatife_dusmez(monkeypatch):
    ...
    async def akis():
        await worker.release()   # hic acilmadan birak
        await worker.acquire()   # sonraki is
    asyncio.run(akis())
    assert worker._count == 1
...
def test_agent_uri_yoksa_sessiz_no_op(monkeypatch):
    ...
    asyncio.run(worker.set_protection(True))   # patlamamali
...
def test_acilirken_ttl_gonderilir_kapanirken_gonderilmez(monkeypatch):
    ...
    asyncio.run(worker.set_protection(True))
    asyncio.run(worker.set_protection(False))
